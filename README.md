...
| **工作台统计看板** | 项目/接口/用例/关联/执行记录统计卡片 + 最近执行趋势图（ECharts）+ 通过率 + 快捷入口 | M7 |
| **主流后台布局** | 侧边栏折叠 + 面包屑 + **多标签页导航**（keep-alive 缓存）+ 统一页头/统计卡片组件 | M7 |
...
│  pt-web      异步任务编排（@Async 导入/分析）+ 工作台统计     │
...
└── frontend/                   # 前端（Vue3 + Vite + TS + Element Plus）
    ├── src/components/
    │   ├── Layout.vue          # 主布局（侧边栏/面包屑/多标签页/keep-alive）
    │   ├── PageHeader.vue      # 通用页头组件
    │   └── StatCard.vue        # 统计卡片组件
    ├── src/styles/global.css   # 全局样式（圆角/阴影/表格美化）
    └── src/views/
        ├── Dashboard.vue       # 工作台（统计看板/趋势图/快捷入口）
...
- [x] **M7** 前端布局重构（侧边栏/面包屑/多标签页/keep-alive）+ 工作台统计看板
